...
export interface ArchiveFilterListBoxProps<T extends { name: string; value?: string }> {
  className?: string;
  lists: T[];
  onChange?: (value: T) => void;
  defaultValue?: T;
  onOpenRarityModal?: () => void; // onOpenModal générique
}
...
function ArchiveFilterListBox<T extends { name: string; value?: string }>({
  ...
  const handleChange = (value: T) => {
    setSelected(value);
    if (value.value && onOpenRarityModal && ["rarity","color","feature","set"].includes(value.value)) {
      onOpenRarityModal();
      return;
    }
            ...
            <Listbox.Options className="absolute end-0 w-52 py-1 mt-2 overflow-auto text-sm text-neutral-900 dark:text-neutral-200 bg-white rounded-xl shadow-lg max-h-96 ring-1 ring-black ring-opacity-5 focus:outline-none dark:bg-neutral-900 dark:ring-neutral-700 z-50">
              {lists.map((item, index) => (
                <Listbox.Option
                  key={index}
                  className={({ active }) =>
                    `${active ? "text-primary-700 dark:text-neutral-200 bg-primary-50 dark:bg-neutral-700" : ""} cursor-default select-none relative py-2 ps-10 pe-4`
                  }
                  value={item}
                >
                  {({ selected }) => (
