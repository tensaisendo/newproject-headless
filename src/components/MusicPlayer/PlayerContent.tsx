  ...
  const renderLeft = () => {
    return (
      <div className="mr-2 flex items-center flex-grow lg:flex-shrink-0 lg:basis-52 overflow-hidden">
        ...
        >
          <div className="absolute start-0 w-10 h-10 sm:w-12 sm:h-12 flex-shrink-0 rounded-full overflow-hidden z-0 shadow">
            <NcImage
              alt={title || "post image"}
              fill
              sizes="3rem"
              containerClassName={`absolute inset-0 transition-transform nc-animation-spin rounded-full z-0 overflow-hidden ${
                playing ? "playing" : ""
              }`}
        ...
        >
          <div className="flex flex-grow items-center justify-evenly text-neutral-500 dark:text-neutral-300 max-w-xs sm:max-w-sm md:max-w-md ">
            <div className="w-12 flex justify-center">
              {databaseId && <NcBookmark postDatabseId={databaseId} />}
            </div>
            {renderBackwards10S()}
            {renderButtonControl()}
            {renderForwards15S()}
